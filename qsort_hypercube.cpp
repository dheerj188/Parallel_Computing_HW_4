...
void HyperCube_Class::HyperCube_QuickSort() {
    ...
    for (k = dimension; k > 0; k--) {
        ...
        if (nbr_k > my_id) {
            // Send number of elements greater than pivot
            MPI_Send(&list_size_gt, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);

            // Receive number of elements less than or equal to pivot
            ...
            // Receive number of elements greater than pivot from neighbor
            MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD, &status);

            // Send number of elements <= pivot to neighbor
            ...
            // Send local elements <= pivot to neighbor
